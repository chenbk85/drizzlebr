...
AC_DEFUN([PANDORA_WARNINGS],[
  ...
  AS_IF([test "x$TARGET_OSX" = "xtrue"],[ac_cv_warnings_as_errors=no])
  ...
  AS_IF([test "$GCC" = "yes"],[
    ...
    AC_CACHE_CHECK([whether it is safe to use -fdiagnostics-show-option],
      ...
      [save_CFLAGS="$CFLAGS"
       CFLAGS="-fdiagnostics-show-option ${AM_CFLAGS} ${CFLAGS} -Werror"
       AC_COMPILE_IFELSE(
         [AC_LANG_PROGRAM([],[])],
    ...
    AC_CACHE_CHECK([whether it is safe to use -floop-parallelize-all],
      ...
      [save_CFLAGS="$CFLAGS"
       CFLAGS="-floop-parallelize-all -Werror ${AM_CFLAGS} ${CFLAGS}"
       AC_COMPILE_IFELSE(
         [AC_LANG_PROGRAM([],[])],
    ...
    AS_IF([test "$INTELCC" = "yes"],[
      ...
      AC_CACHE_CHECK([whether it is safe to use -Wextra],
        ...
        [save_CFLAGS="$CFLAGS"
         CFLAGS="${W_FAIL} -pedantic -Wextra -Werror ${AM_CFLAGS} ${CFLAGS}"
         AC_COMPILE_IFELSE([
           AC_LANG_PROGRAM(
           [[
#include <stdio.h>
...
#include <stdio.h>
      ...
      AC_CACHE_CHECK([whether it is safe to use -Wlogical-op],
        ...
        [save_CFLAGS="$CFLAGS"
         CFLAGS="${W_FAIL} -pedantic -Wlogical-op ${AM_CFLAGS} ${CFLAGS} -Werror"
         AC_COMPILE_IFELSE([
           AC_LANG_PROGRAM(
           [[
#include <stdio.h>
      ...
      AC_CACHE_CHECK([whether it is safe to use -Wredundant-decls from C++],
        ...
        [AC_LANG_PUSH(C++)
         save_CXXFLAGS="${CXXFLAGS}"
         CXXFLAGS="${W_FAIL} -pedantic -Wredundant-decls ${AM_CXXFLAGS} -Werror"
         AC_COMPILE_IFELSE(
           [AC_LANG_PROGRAM([
...
template <> void C<int>::foo();
      ...
      AC_CACHE_CHECK([whether it is safe to use -Wattributes from C++],
        ...
        [AC_LANG_PUSH(C++)
         save_CXXFLAGS="${CXXFLAGS}"
         CXXFLAGS="${W_FAIL} -pedantic -Wattributes -fvisibility=hidden ${AM_CXXFLAGS} -Werror"
         AC_COMPILE_IFELSE(
           [AC_LANG_PROGRAM([
...
inline const EnumDescriptor* GetEnumDescriptor<Table_TableOptions_RowType>() {
  return Table_TableOptions_RowType_descriptor();
      ...
      AC_CACHE_CHECK([whether it is safe to use -Wno-attributes],
        ...
        [save_CFLAGS="$CFLAGS"
         CFLAGS="${W_FAIL} -pedantic -Wno_attributes_ ${AM_CFLAGS} ${CFLAGS} -Werror"
         AC_COMPILE_IFELSE([
           AC_LANG_PROGRAM(
           [[
#include <stdio.h>
  ...
  AS_IF([test "$SUNCC" = "yes"],[
    ...
    AC_CACHE_CHECK([whether E_PASTE_RESULT_NOT_TOKEN is usable],
      ...
      [
        save_CFLAGS="${CFLAGS}"
        CFLAGS="-errwarn=%all -erroff=E_PASTE_RESULT_NOT_TOKEN ${CFLAGS} -Werror"
        AC_COMPILE_IFELSE(
          [AC_LANG_PROGRAM([
            AC_INCLUDES_DEFAULT
          ],[
